...
| 产品交互（页面浏览、派发/配对/验收是否成功，见 `usage-events.md`） | 是（「帮助改进刻迹」默认开启，可在「数据与隐私」关闭） | 是 | 否 | 分析、App 功能 |

- 追踪：**否**。没有第三方 SDK，没有广告标识符。使用统计自建（Valley `POST /events`），30 天后删除，不含任务标题、提示词、仓库路径或账号标识。
